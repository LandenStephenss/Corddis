...
class Shard extends EventEmitter {
  ...
  _onWSMessage(message) {
    ...
    switch (packet.t) {
      ...
      case 'GUILD_MEMBERS_CHUNK': {
        ...
        const queue = this.guildMembersQueue.get(guild.id);
        if (data.not_found) {
   ...
   * @arg {number} limit Number of members to request (0 for no limit)
   * @arg {RequestGuildMembersOptions} [options] Request guild membesr options
   * @returns {Promise<void>}
   */
  requestGuildMembers(guildID, limit, options = {}) {
    this.sendWS(8, {
      guild_id: guildID,
      query: options.query || '',
    ...
    return new Promise((resolve, reject) => {
      this.guildMembersQueue.set(guildID, {
        resolve,
        reject,
        notFound: [],
  ...
  _clearGuildMembersQueue(guildID) {
    const queue = this.guildMembersQueue.get(guildID);
    ...
    this.guildMembersQueue.delete(guildID);
  }
