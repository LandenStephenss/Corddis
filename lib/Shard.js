...
class Shard extends EventEmitter {
    ...
    switch (packet.t) {
      ...
      case 'MESSAGE_UPDATE': {
        ...
        if (!message) {
          this.emit('messageUpdate', {
            newMessage: new Message(data, client),
            oldMessage: null
          });
          break;
        }
      ...
      case 'MESSAGE_DELETE': {
        ...
        this.emit('messageDelete', {
          message,
          channel,
          guild: data.guild_id ? client.guilds.get(data.guild_id) || {
            id: data.guild_id
          } : null
        });
        break;
  ...
  emit(event, data) {
    this.client.gateway.emit(event, data, this);
