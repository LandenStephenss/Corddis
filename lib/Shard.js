...
class Shard extends EventEmitter {
  ...
  identify(options) {
    ...
    this.sendWS(constants.IDENTIFY, {
  ...
  _onWSClose(code, reason) { // eslint-disable-line no-unused-vars
    switch (code) {
      ...
      case constants.INVALID_VERSION: {
        break;
      }

      case constants.INVALID_INTENTS: {
        break;
      }

      case constants.DISALLOWED_INTENTS: {
        break;
      }
    }
  }
  ...
  heartbeat() {
    ...
    this.sendWS(constants.HEARTBEAT, this.seq);
  }
  ...
  updateStatus(options) {
    ...
    this.sendWS(constants.STATUS_UPDATE, {
      afk: !!options.afk,
      game,
  ...
  requestGuildMembers(guildID, options = {}) {
    this.sendWS(constants.REQUEST_GUILD_MEMBERS, {
      guild_id: guildID,
      query: options.query || '',
