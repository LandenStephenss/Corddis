...
class Guild extends Base {
  ...
  addMemberRole(memberID, roleID) {
    return this.shard.client.rest.addGuildMemberRole(this.id, memberID, roleID);
  }
  ...
  /**
   * Get this guild's prune count
   * @arg {number} [days] Number of prune days
   * @returns {GuildPruneObject}
   */
  getPruneCount(days) {
    return this.rest.getGuildPruneCount(this.id, days);
  }
   ...
   */
  leave() {
    return this.shard.client.rest.leaveGuild(this.id);
  }

  /**
   * Prune this guild's members
   * @arg {number} [days] Number of prune days
   * @returns {Promise<GuildPruneObject>}
   */
  pruneMembers(days) {
    return this.client.rest.pruneGuildMembers(this.id, days);
  }
  ...
  removeMemberRole(memberID, roleID) {
    return this.shard.client.rest.removeGuildMemberRole(
      this.id,
      memberID,
      roleID
    );
  ...
  unbanUser(userID, reason) {
    return this.shard.client.rest.unbanGuildUser(this.id, userID, reason);
  }
