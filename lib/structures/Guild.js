...
class Guild extends Base {
  ...
  constructor(data, shard) {
    ...
    const members = this.members = new Collection(Member, data.members, this);

    for (const voiceState of data.voice_states) {
      const member = members.get(voiceState.user_id);
  ...
  get boosters() {
    return this.members.filter((member) => member.premiumSince !== null);
  }
}
