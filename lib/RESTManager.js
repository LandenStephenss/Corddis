...
class RESTManager {
  ...
  createChannelMessageReaction(channelID, messageID, emoji) {
    ...
    return this.request({
      auth: true,
      endpoint: endpoints.userReaction(channelID, messageID, reaction, '@me'),
      method: 'PUT'
    });
  }
  ...
  async getChannelMessageReactionUsers(channelID, messageID, emoji, options) {
    ...
    const users = await this.request({
      auth: true,
      endpoint: endpoints.messageReaction(channelID, messageID, reaction),
      method: 'GET',
      query: options
    });
   ...
   * @returns {Promise<void>}
   */
  removeChannelMessageReaction(channelID, messageID, emoji, userID = '@me') {
    const reaction = encodeURIComponent(emoji);

    return this.request({
      auth: true,
      endpoint: endpoints.userReaction(channelID, messageID, reaction, userID),
      method: 'DELETE'
    });
  }
