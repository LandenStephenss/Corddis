...
class RESTManager {
  ...
  constructor(client, options = {}) {
    this.client = client;
    this.version = options.version || versions.REST_VERSION;
  }
  ...
  addGuildMemberRole(guildID, memberID, roleID) {
    return this.request({
      auth: true,
      endpoint: endpoints.guildMemberRole(guildID, memberID, roleID),
      method: 'PUT'
    });
  }
   ...
   * @arg {MessageContentOptions} options Message create options
  ...
  async editChannelMessage(channelID, messageID, options) {
    const data = await this.request({
      ...
      body: {
        ...options,
        flags: options.suppressEmbeds ? messageFlags.SUPPRESS_EMBEDS : 0,
      },
      endpoint: endpoints.channelMessage(channelID, messageID),
      method: 'PATCH'
    });
  ...
  /**
   * Get the prune count a guild
   * @arg {string} guildID The guild's ID
   * @arg {number} [days] Number of days of prune
   * @returns {Promise<GuildPruneObject>}
   */
  getGuildPruneCount(guildID, days = 7) {
    return this.request({
      auth: true,
      endpoint: endpoints.guildPrune(guildID),
      method: 'GET',
      query: {
        days
      }
  ...
  leaveGuild(guildID) {
    return this.request({
      auth: true,
      endpoint: endpoints.meGuild(guildID),
      method: 'DELETE'
    });
  }
  ...
  /**
   * Prune a guild's members
   * @arg {string} guildID The guild's ID
   * @arg {number} [days] Number of prune days
   */
  pruneGuildMembers(guildID, days = 7) {
    return this.request({
      auth: true,
      endpoint: endpoints.guildPrune(guildID),
      method: 'POST',
   ...
   * @returns {Promise<void>}
   */
  unbanGuildUser(guildID, userID, reason) {
    return this.request({
      auth: true,
      endpoint: endpoints.guildBan(guildID, userID, reason),
      method: 'DELETE'
    });
  }
...
/**
 * @typedef {GetSearchOptions} ChannelMessagesOptions
...
/**
 * @typedef {MessageContentOptions} MessageEditOptions
 * @prop {boolean} [suppressEmbeds] Suppress embeds from the message
 */

/**
 * @typedef {MessageOptions} MessageContentOptions
 * @prop {string} [code] Type of code block to insert the content in
 * @prop {string} [content] Message content
 * @prop {EmbedOptions} [embed] Embed options
 * @prop {AllowedMentionsObject} [allowedMentions] Allowed mentions in the
...
/**
 * @typedef {object} GuildPruneObject
 * @prop {number} [pruned] Number of members that are prunable
 */

/**
 * @typedef {object} AllowedMentionsObject
 * @prop {string[]} [parse] Array of allowed mention types
 * @prop {string[]} [roles] Roles to mention (up to 100)
 * @prop {string[]} [users] Users to mention (up to 100)
 */
