...
class RESTManager {
  ...
  constructor(client, options = {}) {
    this.client = client;
    this.version = options.version || constants.versions.REST_VERSION;
  }
  ...
  addMessageReaction(channelID, messageID, emoji) {
    const reaction = encodeURIComponent(emoji);

    return this.request({
      auth: true,
      endpoint: endpoints.USER_REACTION(channelID, messageID, reaction, '@me'),
      method: 'PUT'
    });
  }
  ...
  removeMessageReaction(channelID, messageID, emoji, userID = '@me') {
    ...
    return this.request({
      auth: true,
      endpoint: endpoints.USER_REACTION(channelID, messageID, reaction, userID),
      method: 'DELETE'
    });
  }
   ...
   */
  async getMessageReactionUsers(channelID, messageID, emoji, options) {
    const reaction = encodeURIComponent(emoji);

    const users = await this.request({
      auth: true,
      endpoint: endpoints.MESSAGE_REACTION(channelID, messageID, reaction),
      method: 'GET',
      query: options
    });
  ...
  pinMessage(channelID, messageID) {
    return this.request({
      auth: true,
      endpoint: endpoints.CHANNEL_PIN(channelID, messageID),
      method: 'PUT'
    });
  }
  ...
  async getPinnedMessages(channelID) {
    const messages = await this.request({
      auth: true,
      endpoint: endpoints.CHANNEL_PINS(channelID),
      method: 'GET'
    });
  ...
  kickGuildMember(guildID, memberID, reason) {
    return this.request({
      auth: true,
      endpoint: endpoints.GUILD_MEMBER(guildID, memberID),
      method: 'DELETE',
      reason
    });
  }
   ...
   * @returns {Promise<void>}
   */
  banGuildMember(guildID, memberID, reason) {
    return this.request({
      auth: true,
      endpoint: endpoints.GUILD_BAN(guildID, memberID),
      method: 'PUT',
      reason
    });
  }
  ...
  unbanGuildUser(guildID, userID) {
    return this.request({
      auth: true,
      endpoint: endpoints.GUILD_BAN(guildID, userID),
      method: 'DELETE'
    });
  }
  ...
  startChannelTyping(channelID) {
    return this.request({
      auth: true,
      endpoint: endpoints.CHANNEL_TYPING(channelID),
      method: 'POST'
    });
  }
  ...
  async getChannelInvites(channelID) {
    const data = await this.request({
      auth: true,
      endpoint: endpoints.CHANNEL_INVITES(channelID),
      method: 'GET'
    });
  ...
  async getGuildInvites(guildID) {
    const data = await this.request({
      auth: true,
      endpoint: endpoints.GUILD_INVITES(guildID),
      method: 'GET'
    });
